const PRODUCTION_HOSTS = ['www.knowhowindia.in', 'knowhowindia.in'];

/** Use same-origin /api when proxied (Vercel or Vite dev) so auth cookies are first-party */
export const usesSameOriginApi = (): boolean => {
  if (typeof window === 'undefined') return false;
  const h = window.location.hostname;
  return (
    PRODUCTION_HOSTS.includes(h) ||
    h === 'localhost' ||
    h === '127.0.0.1'
  );
};
...
export const getBackendBaseUrl = (): string => {
  if (usesSameOriginApi() && typeof window !== 'undefined') {
...
/** Google OAuth callback must stay on Render; initiation can use same-origin proxy */
export const getGoogleAuthStartUrl = (): string => {
  const frontendUrl =
    ...
    window.location.hostname !== '127.0.0.1'
      ...
      : 'http://localhost:8080';
  return `${getApiBaseUrl()}/auth/google?frontend_url=${encodeURIComponent(frontendUrl)}`;
};

const NO_SESSION_REFRESH_PREFIXES = [
  '/auth/login',
  '/auth/signup/',
...
class ApiClient {
  ...
  private shouldAttemptSessionRefresh(endpoint: string): boolean {
    ...
    return !NO_SESSION_REFRESH_PREFIXES.some((prefix) => path.startsWith(prefix));
  }
  ...
  private async request<T>(
    ...
    try {
      ...
      // Handle 401/403 — refresh session only for authenticated API calls (not login/signup)
      if (response.status === 401 || response.status === 403) {
        if (!this.shouldAttemptSessionRefresh(cleanEndpoint)) {
...
/** @deprecated Use getBackendBaseUrl() */
